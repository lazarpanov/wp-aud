...
public class InMemoryShoppingCartRepository {
    public Optional<ShoppingCart> findById(Long id) {
        return DataHolder.shoppingCarts.stream().filter(shoppingCart -> shoppingCart.getId().equals(id)).findFirst();
    }

    public Optional<ShoppingCart> findByUsernameAndStatus(String username, ShoppingCartStatus status) {
        return DataHolder.shoppingCarts.stream().filter(shoppingCart -> shoppingCart.getUser().getUsername().equals(username) && shoppingCart.getStatus().equals(status)).findFirst();
    }
